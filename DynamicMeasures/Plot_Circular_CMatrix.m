...
if nargin<1 % the indispensable input arguments are not provided
    ...
    % Plotting parameters
    param.linethick = [0.5 5];      % Lines thickness
    ...
    param.intringwidth = 0.015;   % Inter ring distance
end
...
colorstrings = strvcat('colormaps_colors(''manitou'',256)',...
...
switch hemiVar
    case 1
        theta = linspace(pi/2+ang,3/2*pi-ang,(N/2))'; %theta(end) = [];
        theta2 = linspace(pi/2-ang,-pi/2+ang,(N/2))'; %theta2(end) = [];   
        theta = [theta;theta2];
    otherwise
    ...
    for i = 1:length(row)
        if row(i) ~= col(i)
            if abs(row(i) - col(i)) - length(LabNet)/2 == 0
                ...
                if directed
                    if invbool
                        if (row(i) <= Nstruct/2) & col(i) <= Nstruct/2
                            colo = Val2colors(flipdim(t,1),cl_left);
                        elseif (row(i) > Nstruct/2) & col(i) > Nstruct/2
...
%     case 1
        ipos       = xy_text(:,1) > 0;
        s.t        = zeros(Nstruct,1);
        s.t( ipos) = text(xy_text(ipos,1).*1.05, xy_text(ipos,2).*1.05, stNames(ipos), 'FontSize',12,'FontName','Helvetica');
